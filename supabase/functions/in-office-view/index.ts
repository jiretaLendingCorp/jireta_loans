...
async function handleSubmit(req: Request) {
  ...
  try {
    ...
    // Ginagawa na rito ang schedule rows (para laging may a-allocate-an ang
    // bayad), pero hindi ipinapakita hangga't hindi active ang loan at
    // ire-rebase ang due dates sa petsa ng release (startLoanPaymentSchedule).
    const scheduleRows: {
      loan_id: string;
