// supabase/functions/disbursements-gcash/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    const { loan_id, gcash_number } = body;
    ...
    await writeAuditLog({
      ...
      newValues: { loan_id, gcash_number, amount, xendit_id: xenditResult.id },
    });

    await sendPushNotification({
      userId: loan.lender_id,
      title: 'Loan Disbursed via GCash',
      body: `Your loan of ₱${amount.toLocaleString()} has been disbursed to ${gcash_number}.`,
      type: 'disbursement',
      referenceId: loan_id,
    ...
    console.error('disbursements-gcash error:', err);
