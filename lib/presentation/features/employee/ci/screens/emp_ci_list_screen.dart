...
class _EmpCiListScreenState extends ConsumerState<EmpCiListScreen> {
    ...
    return WebScaffold(
      ...
      actions: [
        Container(
          ...
          child: IconButton(
              onPressed: () => ref.read(empCiProvider.notifier).fetch(),
        ...
        children: [
          ...
          if (state.totalPages > 1)
            TablePagination(
              ...
              onPageChange: (p) =>
                  ref.read(empCiProvider.notifier).fetch(page: p),
            ),
        ],
      ),
    );
  ...
  Widget _buildFilterBar(EmpCiState state) {
    return Container(
  ...
  Widget _buildList(BuildContext context, EmpCiState state) {
    ...
    final items = q.isEmpty
        ...
        : state.items
            .where((ci) =>
                ci.loanNumber.toString().toLowerCase().contains(q) ||
                ci.borrowerName.toString().toLowerCase().contains(q) ||
                ci.riderName.toString().toLowerCase().contains(q))
            .toList();
    ...
    if (items.isEmpty) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Text('Try adjusting search or status filter',
              style:
    ...
    return ListView.separated(
      ...
      itemBuilder: (ctx, i) {
        ...
        return _CiCard(
          ...
          ci: ci,
          onTap: () =>
              context.go(RouteConstants.empCiDetails.replaceFirst(':id', ci.id)),
        );
      },
    );
  }
      ...
      child: Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(
              ...
              child: const Icon(Icons.search_off_rounded,
                  size: 40, color: AppColors.lenderBlue)),
          const SizedBox(height: 16),
          const Text('No CI assignments found',
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
          const SizedBox(height: 6),
          const Text(
              'Assignments created from loan applications will appear here.',
...
class _CiCardState extends State<_CiCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final isOverdue = ci.deadline != null &&
        (ci.deadline as DateTime).isBefore(DateTime.now()) &&
        status != 'completed';
                  ...
                  Wrap(
                    ...
                    children: [
                      _Meta(
                          ...
                          text: ci.riderName?.toString().isNotEmpty == true
                              ? ci.riderName
                              : 'Unassigned'),
                      _Meta(
                          icon: Icons.person_outline_rounded,
                          text: ci.borrowerName ?? '—'),
                      _Meta(
                        icon: Icons.event_rounded,
                        text: ci.deadline != null
