...
class _AuditState {
  ...
  const _AuditState({this.logs = const [], this.isLoading = false, this.currentPage = 1, this.totalPages = 1});
  _AuditState copyWith({List<Map<String, dynamic>>? logs, bool? isLoading, int? currentPage, int? totalPages}) =>
...
class _AuditNotifier extends StateNotifier<_AuditState> {
  ...
  Future<void> fetch({int page = 1, String? action, String? performedBy}) async {
    ...
    try {
      final res = await _ds.getAuditLogs(page: page, action: action, performedBy: performedBy);
      final logs = (res['data'] as List? ?? []).cast<Map<String, dynamic>>();
      final meta = res['meta'] as Map<String, dynamic>? ?? {};
      state = state.copyWith(logs: logs, isLoading: false, currentPage: meta['page'] as int? ?? 1, totalPages: meta['total_pages'] as int? ?? 1);
...
class _HmAuditLogsScreenState extends ConsumerState<HmAuditLogsScreen> {
  ...
  final _actions = ['login', 'logout', 'loan_created', 'loan_approved', 'loan_rejected', 'payment', 'user_created', 'password_changed', 'settings_changed', 'report_export'];
  ...
  Widget build(BuildContext context) {
    ...
    return WebScaffold(
      title: 'Audit Logs',
      actions: [
        IconButton(onPressed: () => ref.read(_auditProvider.notifier).fetch(), icon: const Icon(Icons.refresh), tooltip: 'Refresh'),
          ...
          children: [
            Expanded(
              child: TextField(
                ...
                onChanged: (v) => ref.read(_auditProvider.notifier).fetch(performedBy: v.isEmpty ? null : v),
              ),
            ),
            const SizedBox(width: 12),
              ...
              child: Container(
                ...
                child: DropdownButton<String?>(
                  ...
                  items: [
                    ...
                    ..._actions.map((a) => DropdownMenuItem(value: a, child: Text(_capitalize(a.replaceAll('_', ' '))))),
                  ],
                  onChanged: (v) {
        ...
        child: Card(
          ...
          child: Column(
            children: [
              ...
              ...state.logs.asMap().entries.map((e) => _buildRow(e.value, e.key.isEven)),
            ],
          ),
        ),
      );
              ...
              children: [
                Expanded(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(color: _actionColor(action).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(4)),
                    child: Text(_capitalize(action.replaceAll('_', ' ')), style: TextStyle(fontSize: 12, color: _actionColor(action), fontWeight: FontWeight.w500)),
                  ),
                ),
                Expanded(
                  flex: 3,
                  child: Row(
                    children: [
                      ...
                      Text('${user['first_name'] ?? ''} ${user['last_name'] ?? ''}'.trim(), style: const TextStyle(fontSize: 13)),
                    ],
                  ),
                ),
                Expanded(flex: 2, child: Text(log['table_name'] as String? ?? '-', style: const TextStyle(fontSize: 13, color: AppColors.textSecondary))),
                Expanded(flex: 2, child: Text(_formatDateTime(log['created_at']), style: const TextStyle(fontSize: 12, color: AppColors.textSecondary))),
                Expanded(flex: 1, child: Icon(isExpanded ? Icons.expand_less : Icons.expand_more, color: AppColors.textSecondary, size: 20)),
              ],
            ),
          ),
        ),
                ...
                if (log['old_values'] != null)
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('BEFORE', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: AppColors.error)),
                        const SizedBox(height: 8),
                        Container(
                          padding: const EdgeInsets.all(12),
                          decoration: BoxDecoration(color: AppColors.errorLight, borderRadius: BorderRadius.circular(8)),
                          child: Text(log['old_values'].toString(), style: const TextStyle(fontSize: 12, fontFamily: 'monospace')),
                        ),
                      ],
                    ),
                  ),
                if (log['old_values'] != null && log['new_values'] != null) const SizedBox(width: 16),
                if (log['new_values'] != null)
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('AFTER', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: AppColors.success)),
                        const SizedBox(height: 8),
                        Container(
                          padding: const EdgeInsets.all(12),
                          decoration: BoxDecoration(color: AppColors.successLight, borderRadius: BorderRadius.circular(8)),
                          child: Text(log['new_values'].toString(), style: const TextStyle(fontSize: 12, fontFamily: 'monospace')),
                        ),
                      ],
                    ),
                  ),
  ...
  Widget _buildPagination(_AuditState state) => Container(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            IconButton(onPressed: state.currentPage > 1 ? () => ref.read(_auditProvider.notifier).fetch(page: state.currentPage - 1) : null, icon: const Icon(Icons.chevron_left)),
            Text('Page ${state.currentPage} of ${state.totalPages}'),
            IconButton(onPressed: state.currentPage < state.totalPages ? () => ref.read(_auditProvider.notifier).fetch(page: state.currentPage + 1) : null, icon: const Icon(Icons.chevron_right)),
          ],
        ),
      );

  Color _actionColor(String action) {
    if (action.contains('create') || action.contains('login')) return AppColors.success;
    if (action.contains('delete') || action.contains('reject') || action.contains('suspend')) return AppColors.error;
    if (action.contains('update') || action.contains('password')) return AppColors.warning;
    if (action.contains('export') || action.contains('report')) return AppColors.info;
