      ...
      child: Column(children: [
        ...
        if (isFiltered) ...[
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            OutlinedButton.icon(onPressed: () { _searchCtrl.clear(); ref.read(hmCiProvider.notifier).setStatus('all'); setState(() {}); }, icon: const Icon(Icons.clear_all_rounded, size: 16), label: const Text('Clear filters')),
            const SizedBox(width: 10),
            ElevatedButton.icon(onPressed: () => ref.read(hmCiProvider.notifier).fetch(), icon: const Icon(Icons.refresh_rounded, size: 16), label: const Text('Refresh'), style: ElevatedButton.styleFrom(backgroundColor: AppColors.deepNavy, foregroundColor: Colors.white)),
          ]),
        ],
      ]),
    );
