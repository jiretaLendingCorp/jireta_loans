...
class _HmRiderListScreenState extends ConsumerState<HmRiderListScreen> {
  ...
  void _onDateRangeChanged(DateTimeRange? r) {
    ...
    ref.read(hmRiderProvider.notifier).setDateRange(
  ...
  Widget _table(HmRiderState state) => SingleChildScrollView(
        padding: const EdgeInsets.all(16),
        child: Column(
          ...
          children: [
            ResponsiveListCard(
              ...
              rows: state.riders.map((e) => _row(e)).toList(),
            ),
            ...
            RecordsPagination(
              ...
              onPageChange: (p) =>
                  ref.read(hmRiderProvider.notifier).setPage(p),
            ),
          ],
        ),
      );
