...
import '../../collections/providers/hm_collection_provider.dart';
...
class _PaymentState {
  ...
  _PaymentState withDateRange(String? from, String? to) => _PaymentState(
        payments: payments,
        isLoading: isLoading,
        error: error,
...
class _HmPaymentListScreenState extends ConsumerState<HmPaymentListScreen> {
  ...
  void _onMethodTap(String key) {
    ...
    ref.read(_hmPaymentProvider.notifier).setFilters(method: key, search: term);
  }
  ...
  void _onSearchChanged(String v) {
    ...
    _searchDebounce = Timer(const Duration(milliseconds: 400), () {
      final term = v.trim();
      if (_pendingPayments) {
        ref.read(hmCollectionProvider.notifier).setFilters(search: term);
      } else {
        ref.read(_hmPaymentProvider.notifier).setFilters(search: term);
      }
    });
  }
  ...
  void _applyDateRangeToCollections() {
    final r = _dateRange;
    ref.read(hmCollectionProvider.notifier).setDateRange(
  ...
  void _onDateRangeChanged(DateTimeRange? r) {
    ...
    ref.read(_hmPaymentProvider.notifier).setDateRange(
          r == null ? null : SearchDateFilter.fromParam(r.start),
          r == null ? null : SearchDateFilter.toParam(r.end),
        );
  ...
  void _refresh() {
    ...
    ref.read(_hmPaymentProvider.notifier).fetch();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final collectionState =
        _pendingPayments ? ref.watch(hmCollectionProvider) : null;
  ...
  Widget _buildToolbar(_PaymentState state, HmCollectionState? collectionState) {
  ...
  Widget _buildPaymentMethodFilter(_PaymentState state) {
  ...
  List<Widget> _buildPaymentsSection(_PaymentState state) {
    ...
    if (state.isLoading && state.payments.isEmpty) {
      ...
      children.add(_buildError(
        state.error!,
        onRetry: () => ref.read(_hmPaymentProvider.notifier).fetch(),
      ));
    ...
    if (state.totalPages > 1) {
      ...
      children.add(_buildPagination(
        ...
        onPage: (p) => ref.read(_hmPaymentProvider.notifier).fetch(page: p),
      ));
    }
    return children;
  }

  List<Widget> _buildPendingSection(HmCollectionState state) {
    final children = <Widget>[];
    if (state.isLoading && state.items.isEmpty) {
      ...
      children.add(_buildError(
        state.error!,
        onRetry: () => ref.read(hmCollectionProvider.notifier).fetch(),
      ));
      ...
      children.add(PendingPaymentsTable(
        items: state.items,
        onView: (c) => context.go(
            RouteConstants.hmCollectionDetails.replaceFirst(':id', c.id)),
        onRefresh: () async {
          await ref.read(hmCollectionProvider.notifier).fetch(silent: true);
          await ref.read(_hmPaymentProvider.notifier).fetch(silent: true);
        },
      ));
    }
    if (state.totalPages > 1) {
      ...
      children.add(_buildPagination(
        currentPage: state.currentPage,
        totalPages: state.totalPages,
        onPage: (p) => ref.read(hmCollectionProvider.notifier).fetch(page: p),
      ));
    }
    return children;
  }
    ...
    return ResponsiveRow(
      cells: [
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(lenderName.isEmpty ? '—' : lenderName,
