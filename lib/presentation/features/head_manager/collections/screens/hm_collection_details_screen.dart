...
class HmCollectionDetailsScreen extends ConsumerWidget {
  ...
  Widget _buildPremiumHeader(CollectionAssignmentModel col, NumberFormat fmt, bool isOffice) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: isOffice ? [const Color(0xFF0D1B2A), const Color(0xFF004D40)] : [const Color(0xFF0D1B2A), const Color(0xFF143D2B), const Color(0xFF1B5E20)], begin: Alignment.topLeft, end: Alignment.bottomRight),
        ...
        children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Text(col.lenderName.isNotEmpty ? col.lenderName : isOffice ? 'Office Payment Request' : 'Collection Assignment', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Colors.white))),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                  decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.14), borderRadius: BorderRadius.circular(6)),
                  child: Text(isOffice ? 'OFFICE' : 'RIDER', style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w800, letterSpacing: 0.6)),
                ),
              ]),
              const SizedBox(height: 4),
              Row(children: [
                Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(6)), child: Text(col.loanNumber.isNotEmpty ? col.loanNumber : '—', style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600))),
                const SizedBox(width: 8),
                Text(col.amountCollected != null ? '₱${fmt.format(col.amountCollected!)} collected' : 'Pending collection', style: const TextStyle(color: Colors.white70, fontSize: 12)),
              ]),
            ]),
          ),
          const SizedBox(width: 12),
        ...
        children: steps.asMap().entries.map((e) {
          ...
          return Expanded(
            child: Row(children: [
              ...
              if (!isLast) Expanded(child: Container(height: 2, margin: const EdgeInsets.symmetric(horizontal: 8), decoration: BoxDecoration(color: isDone ? AppColors.riderGreen.withValues(alpha: 0.4) : AppColors.border, borderRadius: BorderRadius.circular(2)))),
            ]),
          );
        }).toList(),
      ),
  ...
  Widget _buildAssignmentCard(CollectionAssignmentModel col, DateFormat dateFmt, bool isOffice) {
    return _PremiumInfoCard(
      ...
      rows: [
        ...
        _Row('Request Type', isOffice ? 'Pay at the Office' : 'Rider Collection', icon: isOffice ? Icons.storefront_rounded : Icons.delivery_dining_rounded),
        ...
        _Row(isOffice ? 'Payment Location' : 'Assigned Rider', isOffice ? 'Office' : col.riderName.isNotEmpty ? col.riderName : 'Unassigned', icon: Icons.delivery_dining_rounded, highlight: !isOffice && col.riderName.isNotEmpty),
  ...
  Widget _buildPaymentCard(CollectionAssignmentModel col, Map<String, dynamic> schedule, NumberFormat fmt, BuildContext context) {
    ...
    return _PremiumInfoCard(
      ...
      rows: [
        _Row('Amount Due', schedule['amount_due'] != null ? '₱${fmt.format((schedule['amount_due'] as num).toDouble())}' : 'N/A', icon: Icons.request_quote_rounded, highlight: true),
        _Row('Amount Collected', col.amountCollected != null ? '₱${fmt.format(col.amountCollected!)}' : 'Not yet collected', icon: Icons.savings_rounded, highlight: col.amountCollected != null),
        _Row('Due Date', schedule['due_date'] != null ? DateFormat('MMM d, yyyy').format(DateTime.parse(schedule['due_date'])) : 'N/A', icon: Icons.calendar_today_rounded),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          decoration: BoxDecoration(color: AppColors.deepNavy.withValues(alpha: 0.04), borderRadius: const BorderRadius.vertical(top: Radius.circular(14)), border: const Border(bottom: BorderSide(color: AppColors.divider))),
          child: Row(children: [
            Container(width: 36, height: 36, decoration: BoxDecoration(gradient: const LinearGradient(colors: [AppColors.deepNavy, Color(0xFF1A2E4A)]), borderRadius: BorderRadius.circular(9)), child: const Icon(Icons.location_on_rounded, color: Colors.white, size: 18)),
            const SizedBox(width: 10),
            const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Collection Location', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800)), Text('GPS captured on completion', style: TextStyle(fontSize: 11, color: AppColors.textSecondary))]),
            const Spacer(),
            Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: AppColors.riderGreen.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(6)), child: const Text('GEOTAGGED', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.riderGreen))),
          ]),
        ),
        Padding(
          padding: const EdgeInsets.all(16),
          child: Column(children: [
            Container(
              ...
              child: Row(children: [
                Container(width: 36, height: 36, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: AppColors.border)), child: const Icon(Icons.my_location_rounded, size: 18, color: AppColors.deepNavy)),
                const SizedBox(width: 10),
...
class _PremiumInfoCard extends StatelessWidget {
  ...
  const _PremiumInfoCard({required this.title, required this.subtitle, required this.icon, required this.accent, required this.rows});
