...
class _HmAccountUpgradeDetailsScreenState
          ...
          child: ConstrainedBox(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              Container(
                ...
                child: Row(children: [
                  Container(width: 36, height: 36, decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.14), borderRadius: BorderRadius.circular(9)), child: const Icon(Icons.insert_drive_file_rounded, color: Colors.white, size: 18)),
                  const SizedBox(width: 10),
                  const Expanded(child: Text('Document Preview', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 15))),
              ...
              Flexible(child: SingleChildScrollView(padding: const EdgeInsets.all(16), child: DocumentViewer(url: url, height: 540))),
            ]),
          ),
        ),
      );
        ...
        children: [
          ...
          LayoutBuilder(builder: (context, constraints) {
                            ...
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Row(children: [
                                Container(
                                  ...
                                  child: Icon(_iconForDocType(d['document_type']?.toString() ?? ''), size: 18, color: AppColors.deepNavy),
                                ),
                                const SizedBox(width: 10),
                              ...
                              if (d['rejection_notes'] != null && (d['rejection_notes'] as String).isNotEmpty)
                                Container(
                                  ...
                                  decoration: BoxDecoration(color: AppColors.error.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(8), border: Border.all(color: AppColors.error.withValues(alpha: 0.2))),
                                  child: Row(children: [
                                    const Icon(Icons.error_outline_rounded, size: 14, color: AppColors.error),
                                    const SizedBox(width: 6),
                                    Expanded(child: Text(d['rejection_notes'] as String, style: const TextStyle(fontSize: 12, color: AppColors.error))),
                                  ]),
                                ),
                              if (d['file_url'] != null)
                                Padding(
                                  ...
                                  child: SizedBox(
                                    ...
                                    child: OutlinedButton.icon(
                                      ...
                                      label: const Text('View Document', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
                                      style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 10), side: const BorderSide(color: AppColors.deepNavy), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                                    ),
                                  ),
                                ),
                            ]),
                  ...
                  child: Column(
                    children: contacts.map((c) {
                      ...
                      return Container(
                        ...
                        child: Row(children: [
                          Container(width: 36, height: 36, decoration: BoxDecoration(color: AppColors.warning.withValues(alpha: 0.15), shape: BoxShape.circle), child: const Icon(Icons.person_rounded, size: 18, color: AppColors.warning)),
                          const SizedBox(width: 10),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('${m['name'] ?? '—'}', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13)), Text('${m['relationship'] ?? '—'} • ${m['phone_number'] ?? '—'}', style: const TextStyle(fontSize: 12, color: AppColors.textSecondary))])),
                        ]),
                      );
                    }).toList(),
                  ),
            ...
            final rightRail = SizedBox(
              ...
              child: Column(children: [
                Container(
                  decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: pendingDocs.isEmpty ? AppColors.riderGreen.withValues(alpha: 0.3) : AppColors.border), boxShadow: [BoxShadow(color: (pendingDocs.isEmpty ? AppColors.riderGreen : AppColors.deepNavy).withValues(alpha: 0.06), blurRadius: 12, offset: const Offset(0, 4))]),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Container(
                      padding: const EdgeInsets.all(16),
                      decoration: BoxDecoration(
                        gradient: LinearGradient(colors: pendingDocs.isEmpty ? [AppColors.riderGreen, AppColors.riderGreenDark] : [AppColors.deepNavy, const Color(0xFF1A2E4A)]),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(color: AppColors.deepNavy.withValues(alpha: 0.04), borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.border)),
                  child: Row(children: [
                    Container(width: 32, height: 32, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: AppColors.border)), child: const Icon(Icons.shield_rounded, size: 16, color: AppColors.deepNavy)),
                    const SizedBox(width: 10),
                    const Expanded(child: Text('Decisions are logged to the audit trail and notify the lender instantly.', style: TextStyle(fontSize: 11, color: AppColors.textSecondary, height: 1.4))),
                  ]),
                ),
              ]),
            );
  ...
  Widget _buildLenderHero(Map<String, dynamic> lender, String status, int totalDocs, int verified, int pending, Color accent) {
    final fullName = '${lender['first_name'] ?? ''} ${lender['middle_name'] ?? ''} ${lender['last_name'] ?? ''}'.replaceAll(RegExp(r'\s+'), ' ').trim();
    final progress = totalDocs == 0 ? 0.0 : verified / totalDocs;
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        gradient: const LinearGradient(colors: [Color(0xFF0D1B2A), Color(0xFF1A2E4A), Color(0xFF1E3A5F)], begin: Alignment.topLeft, end: Alignment.bottomRight),
      ...
      child: Column(children: [
        Row(children: [
          Stack(children: [
            ProfileAvatar(photoUrl: lender['profile_photo_url'] as String?, name: fullName, color: Colors.white, radius: 36, borderColor: Colors.white, fallback: const Icon(Icons.person_rounded, size: 32, color: AppColors.deepNavy)),
            Positioned(
              bottom: 0,
              right: 0,
              child: Container(width: 22, height: 22, decoration: BoxDecoration(color: accent, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 2)), child: Icon(_iconForStatus(status), size: 12, color: Colors.white)),
            ),
          ]),
          const SizedBox(width: 16),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(fullName.isEmpty ? 'Unknown Lender' : fullName, style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)),
        ...
        ClipRRect(
          ...
          child: LinearProgressIndicator(value: progress, minHeight: 6, backgroundColor: Colors.white.withValues(alpha: 0.18), valueColor: AlwaysStoppedAnimation<Color>(accent)),
        ),
        const SizedBox(height: 6),
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('$pending pending • $verified verified', style: const TextStyle(color: Colors.white70, fontSize: 11)),
          Text('${(progress * 100).toStringAsFixed(0)}% complete', style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700)),
        ]),
      ]),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(color: accent.withValues(alpha: 0.06), border: const Border(bottom: BorderSide(color: AppColors.divider)), borderRadius: const BorderRadius.vertical(top: Radius.circular(14))),
          child: Row(children: [
            ...
            const SizedBox(width: 10),
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: AppColors.textPrimary)), Text(subtitle, style: const TextStyle(fontSize: 11, color: AppColors.textSecondary))]),
          ]),
        ),
